...
class CourseVideo3 extends BaseController
{
    public function index()
    {
        ...
        $first_video_name = $model->getFirstVideoName($videoID_3);

        $video_path = "writable/uploads_video/$first_video_name";

        $first_video_title = $model->getFirstTitle($videoID_3);

        $model = model('App\Models\Video_comment_model');
        $commentIDs= $model->getCommentID($videoID_3);
        if($commentIDs){
    ...
    public function addComment() {
        ...
        if($userID){
            ...
            $model = model('App\Models\Video_comment_model');
            $model-> connect($videoID_3,$commentID);
            $this->index();
        }else{
    ...
    public function addFavourite(){
        ...
        $model-> connect($userID,$videoID_3); //
