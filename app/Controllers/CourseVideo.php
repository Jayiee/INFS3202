...
class CourseVideo extends BaseController
{
    public function index()
    {
        ...
        // $first_video_id= $model->getFirstID($videoID_1);
        $first_video_name = $model->getFirstVideoName($videoID_1);

        $video_path = "writable/uploads_video/$first_video_name";

        $first_video_title = $model->getFirstTitle($videoID_1);

        $model = model('App\Models\Video_comment_model');
        $commentIDs= $model->getCommentID($videoID_1);
        if($commentIDs){
    ...
    public function addComment() {
        ...
        if($userID){
            ...
            $model = model('App\Models\Video_comment_model');
            $model-> connect($videoID_1,$commentID);
            $this->index();
        }else{
    ...
    public function addFavourite(){
        ...
        $model-> connect($userID,$videoID_1); //
